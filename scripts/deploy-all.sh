...
echo ""
    ...
    echo "Use: aws sns subscribe --topic-arn $(aws sns list-topics --region $Region --query 'Topics[?contains(TopicArn, `orders-notifications`)].TopicArn' --output text) --protocol email --notification-endpoint gian.aedo@utec.edu.pe --region $Region"
echo ""
